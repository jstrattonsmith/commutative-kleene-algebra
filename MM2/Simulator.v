(* A step-indexed, monotone, L-extractable Gallina simulator for MM2
   (two-counter machine) programs, indexed by a hand-rolled Godel
   coding of programs as naturals (progOf/codeOf). Builds two disjoint
   enumerable halting sets (A0_MM2/B1_MM2) directly from the simulator,
   with zero Kleene-algebra content anywhere in this file -- everything
   here is about the MM2 execution model and its L-extractability,
   reusable by any other MM2-based project.

   Extracted 2026-08-14 from CKAUndec.Glue.MM2ToKATerm.v, whose
   ...
   The nat<->list mm2_instr encoding below is deliberately hand-rolled
   via embed/unembed (Cantor pairing) rather than stdpp's generic
   Countable machinery: an earlier version used stdpp's Countable (via
   a sum-type injection + the free list_countable instance), which is
   fine for the axiom-free machine-relative approach, but does NOT
   extract to L (L's `extract` tactic needs every function/type
   involved to be built from L.Datatypes-registered primitives, and
   stdpp's positive-based Countable internals aren't part of that
   registry -- confirmed by direct probing: `computable progOf` fails
   to find a `TT` instance). This version *does* extract (see the
   "L-extractability" section at the end of this file), which is what
   the CT_L-based route needs: mm2_outcome_at must be realizable as an
   actual L-term so the eff-insep diagonal argument can be built
   natively in L (reusing SMN_for T_L + LMuRecursion.mu, exactly as
   EffectiveInseparability_L.v already does) instead of needing a
   hand-rolled MM2-level register-packing racer or an output-preserving
   MM-to-MM2 compiler. *)
...
(* --- 2. Plain MM2-execution-model facts, still no KA content. These used
   ...
   R_target_iff_outcome (same source position, CKAUndec.Glue.MM2ToKATerm.v's
